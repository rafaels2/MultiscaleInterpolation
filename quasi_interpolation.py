...
def _calculate_phi(rbf, scale, point, polynom):
    def phi(x, y):
        vector = np.array([x, y])
        return rbf(la.norm(vector - point) / scale) * polynom(point[0], point[1])
    return phi
...
def quasi_scaled_interpolation(scale, original_function, grid_resolution, grid_size, rbf):
    ...
    phis = [_calculate_phi(rbf, scale, np.array([x_i, y_i]), polynom) for x_i, y_i in zip(x, y)]
...
def main():
    ...
    x_test, y_test = generate_grid(10, 3, 2)
