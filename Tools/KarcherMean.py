...
class KarcherMean(object):
    ...
    def _get_step_length(self, x_l):
        ...
        matrices_to_condition = (np.matmul(np.matmul(x_sqrt_inv, a_i), x_sqrt_inv)
        ...
        return 2 / sum(w_i * np.log(c_i) * ((c_i + 1) / (c_i - 1)) for
                       c_i, w_i in zip(conditions, self._weights))
    ...
    def get_average(self, base=None, i=0):
        ...
        exp_param = step_length * sum(w_i * logm(np.matmul(base_inv, a_i))
...
def main():
    _manifold = SymmetricPositiveDefinite()
    _a, _b, _c = (_manifold.gen_point() for _ in range(3))
    _average = KarcherMean(_manifold, [_a, _b, _c], [1, 1, 1]).get_average()

    return _manifold, _a, _b, _c, _average


if __name__ == "__main__":
    man, a, b, c, average = main()
